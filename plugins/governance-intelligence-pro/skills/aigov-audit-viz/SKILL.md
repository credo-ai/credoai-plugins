...
1. **Header** — Credo AI logo, system name, audit date, rigor badge, posture badge, next-audit-date badge
...
4. **Risk Delta View** — two side-by-side 5×5 matrices (Initial / Residual) plus movement table sorted by magnitude
5. **Compliance Scoreboard** — per-regulation cards with obligation status breakdown
6. **Control Effectiveness View** — sortable, expandable table grouped by effectiveness rating
7. **Catalog Drift** — only render if drift data exists
8. **Recommendations** — prioritized action cards with owner-type badges
9. **Trend View** — only render if the audit is in comparison mode with prior audits
10. **Footer** — "Generated by Credo AI Governance Hub · [ISO date]"; read `./docs/credoai/org.md` (local) or `~/.claude/credoai/org.md` (global fallback); if either exists, append " · Prepared for [Organization Name]"
...
For "Effectively Mitigated" residuals, place them in the Low quadrant with a subtle marker (small ✓ in the corner of the pill).
