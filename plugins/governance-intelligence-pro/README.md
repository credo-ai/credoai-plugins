...
2. **Tool inventory and interaction protocol** — which systems hold governance-relevant info (W&B, Confluence, Jira, etc.) *and* how Claude should interact with each (MCP, CLI, API, manual paste, file upload, screenshot). This is a protocol agreement, not a capability check — it tells downstream skills whether to pull the data themselves or phrase a request for the user.
